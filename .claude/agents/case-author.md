...
**Every case owes a workflow file that imports into real n8n, and you must LOOK at it.**

Any registered catalog type exports: `exportWorkflow.js` falls back to `genericNodeSpec()`, which
derives n8n parameters from the descriptor plus your correct answers. So the export will not
simply fail on a canonical type — which means a clean exit is not evidence the file is right.

```bash
npm run workflows:generate -- <slug>     # then READ the parameters it produced
```

Check each of your nodes in the emitted `workflow.n8n.json`. If a node's parameters look thin or
wrongly shaped for what that node really needs in n8n — a Switch without `rules.values[].outputKey`,
a Sheets node without a `{ __rl }` resourceLocator, an expression that could not resolve — that
node needs a hand-written override in `packages/engine/n8nNodeSpecs.js`. Report any you find in
`thingsAHumanShouldCheck` rather than editing that file yourself; it is outside your remit.

**Never swap a canonical type for a legacy alias to make the export look better.** That is
choosing the wrong node for a tooling reason.
