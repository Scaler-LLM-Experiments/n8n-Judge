...
**The node vocabulary is closed.** Every node type you use must already exist in
`packages/catalog/catalog.js`. There are 23. Adding one is a code change in two files and
is outside what you may do.

> If the spec needs a node type that does not exist, **STOP**. Report
> `blocked: true` with `blockedReason` naming the node the spec needs and the closest
> catalog types. Do **not** substitute a near-miss — a case built on the wrong node
> teaches the wrong thing while passing every test.
...
2. **`build.js`** — `nodePalette`, `branches`, `flowSummary`, `flow`, `buildPhases`.
   ...
   - **Every phase must declare its own `pickable`.** The picker's fallback offers only 9
     of the 23 types, so an omitted `pickable` can make a required node unpickable.
