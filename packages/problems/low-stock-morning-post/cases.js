...
export const evalQuestions = [
  {
    id: 'fan-out',
    prompt:
      'A colleague leaves out the step that gathers rows. On Monday eleven beans were low and eleven messages landed. On Friday one bean was low, and their flow matched yours. Why?',
    options: [
    ...
    correctIndex: 2,
    explanation:
      'One run, one trigger, one read. None of those multiply. Items do. The sheet hands its rows on one at a time, and every node after it runs once for each item that reaches it. Monday: eleven rows survive the comparison, so eleven posts. Friday: one item, one post. The same rule, and a result that looks perfectly fine. The bug is broken in proportion to how bad the morning is. It behaves for a fortnight, then floods the channel on the first Monday anybody needed it.',
  },
  {
    id: 'quiet-friday',
    prompt:
    ...
    correctIndex: 0,
    explanation:
      'A node with no items reaching it is never executed, so the gather step and the post do not run and the channel stays quiet. That is correct behaviour, and it is the flow\'s weakest point. Silence is ambiguous. Full shelves, an expired credential, a workflow switched off after a demo and a broken comparison all produce the same nothing. Ops flows people rely on usually post an all-clear on quiet days. Not because anyone reads it, but so that a missing message becomes evidence.',
  },
  {
    ...
    id: 'uncounted',
    prompt:
      'Wednesday. One row has a blank kg_on_hand. Another reads "8 kg", with the unit. Your flow asks one question of every row: is kg_on_hand below reorder_level? What is wrong with asking only that?',
    options: [
      ...
      'Stop the run whenever a kg_on_hand is blank or unreadable, so nobody acts on a shortlist from an unclean sheet',
      'One condition says only "low" or "fine", never "nobody counted this". The rule needs a second arm: below its level, or not a number',
    ],
    correctIndex: 3,
    explanation:
      'The two odd cells look like a parsing problem and are really a design problem. Your rule can say only two things. It sorts every row into low or fine, with no third answer for "nobody counted this". The bean nobody weighed is exactly the one that runs out mid-service. Defaulting a blank to zero is not the fix: zero means somebody counted and found none, blank means nobody looked. Stopping the run is not the fix either. One hurried cell should not cost the other thirty-nine rows their morning.',
  },
  {
    ...
    id: 'late-correction',
    prompt:
      'Friday, 09:15. A barista corrects Decaf Colombia from 12 kg to 0.5, below its level of 4. The 07:30 post went out without it. When does your flow next get a chance?',
    options: [
    ...
    correctIndex: 1,
    explanation:
      'A clock trigger is deaf to the data, and that is mostly what you want. The post arrives at a predictable time, and the channel does not fill with noise. The price is that the flow\'s picture of the store room is exactly as old as the last sweep. Each sweep reads the whole tab afresh, so the corrected row will be there. It is the waiting that costs. On a Friday that is two and a half days, through the weekend the cafes are busiest.',
  },
];
