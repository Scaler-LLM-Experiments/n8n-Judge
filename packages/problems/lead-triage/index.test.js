...
import { leadTriage } from './index.js';

describe('leadTriage problem spec', () => {
  it('has 5 test cases and 2 eval questions', () => {
    expect(leadTriage.testCases).toHaveLength(5);
    expect(leadTriage.evalQuestions).toHaveLength(2);
  });

  it('has 3 branches', () => {
    expect(leadTriage.branches).toHaveLength(3);
  });

  it('every eval question has a valid correctIndex within its options', () => {
    for (const q of leadTriage.evalQuestions) {
  ...
  it('the palette includes every required node type', () => {
    const requiredTypes = new Set(
      leadTriage.nodePalette.filter((n) => !n.isDistractor).map((n) => n.type)
    );
  ...
  it('the palette includes at least one distractor node', () => {
    expect(leadTriage.nodePalette.some((n) => n.isDistractor)).toBe(true);
  });

  it('has 3 build phases covering every required node type', () => {
    expect(leadTriage.buildPhases).toHaveLength(3);
    const phaseTypes = new Set(leadTriage.buildPhases.flatMap((p) => p.nodeTypes));
    for (const t of ['trigger', 'classify', 'chat-gemini', 'parse', 'switch', 'action']) {
      expect(phaseTypes.has(t)).toBe(true);
    }
    for (const p of leadTriage.buildPhases) expect(typeof p.coach).toBe('string');
  });

  it('every node-setup field has exactly one correct option, each with a why', () => {
    for (const [type, setup] of Object.entries(leadTriage.nodeSetup)) {
  ...
  it('every probe has exactly one correct ("by mistake") option', () => {
    for (const [type, probe] of Object.entries(leadTriage.nodeProbes)) {
  ...
  it('every reference graph node has a numeric canvas position', () => {
    for (const node of leadTriage.referenceGraph.nodes) {
