// Editor-only descriptor for @n8n/n8n-nodes-langchain Ollama Chat Model v1.
...
const ollamaChatModel = {
  ...
  usableAsTool: false,
  icon: '/node-icons/ollama-chat-model.svg',
  ...
  aliases: ['ollama', 'local llm', 'llama'],
  docs:
  ...
  source: {
    ...
    directDescriptionImports: [
      ...
      {
        module: '../LMOllama/description',
        names: ['ollamaModel', 'ollamaOptions', 'ollamaDescription'],
    ...
    runtimeImportsExcluded: [
      { module: '@langchain/ollama', names: ['ChatOllamaInput', 'ChatOllama'] },
      {
        module: '@n8n/ai-utilities',
        names: ['makeN8nLlmFailedAttemptHandler', 'N8nLlmTracing', 'proxyFetch'],
      },
      { module: '@utils/chatModelMessageWrapper', names: ['wrapChatModelMessageInput'] },
    ],
  },
  ...
  authoringParity: {
    ...
    topLevelFields: ['credentials.ollamaApi', 'notice', 'model', 'options'],
    optionFields: [
  ...
  dormantExportAudit: {
    ...
    dormantExports: [],
    note: 'Unlike Embeddings Ollama, Ollama Chat imports and renders ollamaOptions.',
  },
  ...
  platformGaps: [
    ...
    'All 20 current Ollama options are authoring metadata only; none changes memory, GPUs, CPU threads, sampling, thinking, safety, or response formatting.',
    'ChatOllama construction, optional bearer headers, proxy fetching, tracing, failure handling, message wrapping, model invocation, and language-model output never run.',
  ],
...
export default ollamaChatModel;
